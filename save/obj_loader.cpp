...
OBJModel::OBJModel(const std::string& fileName)
{
    ...
    std::string sToC = fileName.c_str();
    ...
    if(file.is_open())
    {
        while(file.good())
        {
            ...
            const char* lineCStr = line.c_str();//OpenGL can only read C type strings not C++
    ...
    {
        std::cout <<"Bamboozled"<<sToC<<std::endl;
        std::cerr << "UH OH :( Unable to load mesh: " << fileName << std::endl;
    }
}
...
IndexedModel OBJModel::ToIndexedModel()
{
    ...
    for(unsigned int i = 0; i < numIndices; i++)
    {
        ...
        if(hasUVs)
            currentTexCoord = uvs[currentIndex->uvIndex]; //- -breaks- -
        else
